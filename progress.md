...
| Change | 状态 | 说明 |
|---|---|---|
| entropy-governance | #415 Chat model/Agent routing boundary complete, issues #408-#424 | `.entropy-baseline/latest.json` 已记录实际项目代码熵基线并排除 `external/*`；#410/#411/#412 API error helper 迁移完成；#413 session/scope/ACL、#414 retrieval/rerank/trace construction、#415 model/provider resolution 和 Agent/static routing 已拆为 bounded collaborators，`ChatService` 仍保留 trace DB insert、message/citation/usage persistence 和 SSE shaping；#415 focused suites 81+20+7 pass，API test 1480 pass，API typecheck/lint、OpenSpec strict validate 和 diff check 通过 |
