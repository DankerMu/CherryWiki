...
| Change | 状态 | 说明 |
|---|---|---|
| entropy-governance | #423 url-fetcher worker protocol migration complete, issues #408-#424 | `.entropy-baseline/latest.json` 已记录实际项目代码熵基线并排除 `external/*`；#410/#411/#412 API error helper 迁移完成；#413-#416 API Chat backend 已拆为 bounded collaborators；#417-#419 Web Chat 拆分完成；#420 选定 `packages/python-worker-protocol/`；#421 已新增 `cherry_worker_protocol` lifecycle 包与测试，并完成 Docker/compose/CI/worker venv shared-package visibility wiring；PR-437 fix 补 live-stack smoke shared package 安装、Docker root-context hygiene、protocol failure-boundary tests；#422 已将 `apps/ingestion-worker` lifecycle wrapper 迁移到 shared protocol，保留 ingestion handler/error/result/health 行为；PR-438 R1 修复 ingestion `InternalApiClient` 旧默认签名兼容性；#423 已将 `apps/url-fetcher-worker` lifecycle wrapper 迁移到 shared protocol，并完成两 worker deployability 验证 |
...
1. **熵治理实现批次** — #423 URL fetcher shared protocol 迁移与两 worker deployability 验证已完成；继续 #424 baseline refresh / governance closure
