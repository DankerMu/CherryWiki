# 10. 数据模型与数据库设计

## 1. 设计原则

1. 多租户字段 `tenant_id` 必须存在于核心业务表。
2. Space 是权限和知识隔离的基本单位。
3. Wiki Page 是唯一可检索知识源。
4. 每个可检索对象必须绑定 `page_id`、`page_version` 和 ACL。
5. Graphify 运行结果必须版本化。
6. 原始上传文件只作为 Source Archive，不直接进入检索。

## 2. 核心实体

```mermaid
erDiagram
  tenants ||--o{ users : has
  tenants ||--o{ spaces : has
  users ||--o{ audit_logs : creates
  groups ||--o{ group_members : has
  users ||--o{ group_members : belongs
  spaces ||--o{ wiki_pages : contains
  wiki_pages ||--o{ wiki_page_versions : versions
  spaces ||--o{ source_documents : archives
  file_blobs ||--o{ source_documents : referenced_by
  source_documents ||--o{ ingestion_jobs : processed_by
  spaces ||--o{ graphify_runs : has
  graphify_runs ||--o{ graph_nodes : outputs
  graphify_runs ||--o{ graph_edges : outputs
  wiki_page_versions ||--o{ wiki_chunks : chunks
  wiki_chunks ||--o{ embeddings : vectorized
  graph_nodes ||--o{ graph_edges : connects
  graphify_runs ||--o{ graph_reports : reports
```

## 3. 表设计摘要

### 3.1 `spaces`

代表知识权限域。

关键字段：

- `id`
- `tenant_id`
- `name`
- `slug`
- `docmost_space_id`
- `wiki_repo_path`
- `default_publish_policy`
- `graphify_config`
- `created_at`

### 3.2 `wiki_pages`

页面逻辑主表。

关键字段：

- `id`
- `tenant_id`
- `space_id`
- `page_id`
- `title`
- `slug`
- `status`
- `current_version_id`
- `docmost_page_id`
- `created_by`
- `updated_at`

### 3.3 `wiki_page_versions`

页面版本表。

关键字段：

- `id`
- `page_id`
- `version_no`
- `content_markdown`
- `frontmatter_json`
- `source`：graphify/docmost/manual_merge
- `graphify_run_id`
- `commit_hash`
- `status`
- `created_by`
- `created_at`

### 3.4 `file_blobs` + `source_documents`

物理文件去重层 + Space 内资料引用和业务状态层。同一文件上传到不同 Space 时，`file_blobs` 只存一份（`tenant_id + sha256` 唯一），`source_documents` 每个 Space 各一条引用记录。

**`file_blobs`** — 物理文件去重层：

- `id`
- `tenant_id`
- `sha256`（内容去重键）
- `size_bytes`
- `mime_type`
- `storage_uri`（MinIO 存储路径）

**`source_documents`** — Space 内资料引用和业务状态层：

- `id`
- `tenant_id`
- `space_id`
- `file_blob_id`（引用 `file_blobs.id`）
- `filename`（原始文件名）
- `uploader_id`
- `source_type`：upload / url / docmost_attachment
- `classification`
- `status`：uploaded / processing / parsed / failed
- `parsed_uri`（解析产物 MinIO 路径）
- `metadata_json`

### 3.5 `graphify_runs`

Graphify 运行记录。

关键字段：

- `id`
- `tenant_id`
- `space_id`
- `trigger_type`
- `mode`
- `status`
- `input_version`
- `output_version`
- `graph_json_uri`
- `wiki_output_uri`
- `report_uri`
- `started_at`
- `completed_at`
- `error_json`

### 3.6 `graph_nodes`

图谱节点。

关键字段：

- `id`
- `tenant_id`
- `space_id`
- `graphify_run_id`
- `node_key`
- `label`
- `type`
- `community_id`
- `page_id`
- `page_version_id`
- `source_refs_json`
- `acl_json`

### 3.7 `graph_edges`

图谱关系。

关键字段：

- `id`
- `tenant_id`
- `space_id`
- `graphify_run_id`
- `source_node_id`
- `target_node_id`
- `relation_type`
- `confidence_label`
- `raw_confidence_score`（Graphify 原始分数）
- `effective_confidence_score`（平台归一化，RAG 排序使用）
- `evidence_count`
- `evidence_refs_json`
- `acl_json`

### 3.8 `wiki_chunks`

页面切片。`source_chain_json` 在索引时预计算，检索时零成本读取（方案 B，见 Doc 09 §8.4）。

关键字段：

- `id`
- `tenant_id`
- `space_id`
- `wiki_page_pk`
- `page_version_id`
- `section_id`
- `chunk_index`
- `content`
- `content_hash`
- `token_count`
- `index_status` / `index_snapshot_id`
- `injection_risk`（BOOLEAN，ingestion-worker 检测到 prompt injection 模式时标记 true，检索 rerank 降权 ×0.3）
- `source_chain_json`（预计算溯源链：source_document_ids + graph_node_ids + graph_edge_ids + chain_confidence）
- `acl_json`

### 3.9 `embeddings`

Embedding 表。Phase 1 可用 pgvector。

关键字段：

- `id`
- `chunk_id`
- `model_id`
- `embedding`
- `created_at`

## 4. ACL Envelope

可检索对象统一 ACL：

```json
{
  "tenant_id": "tenant_001",
  "space_id": "space_rd",
  "allowed_group_ids": ["group_rd", "group_arch"],
  "classification": "internal",
  "page_id": "rd.auth.sso",
  "page_version": 12
}
```

所有索引对象必须保存 ACL 快照，避免检索时跨表查询过重。

## 5. 索引策略

### 5.1 唯一索引

- `wiki_pages(tenant_id, space_id, page_id)`
- `file_blobs(tenant_id, sha256)`
- `source_documents(space_id, file_blob_id)`
- `graph_nodes(tenant_id, space_id, graphify_run_id, node_key)`

### 5.2 检索索引

- `wiki_chunks`：全文索引。
- `embeddings`：vector index。
- `graph_nodes(label)`：trigram 或全文。
- `graph_edges(source_node_id, target_node_id)`。
- `audit_logs(tenant_id, created_at)`。

## 5A. Embedding 模型管理策略

### Phase 1 约束

1. 每个 tenant 同一时间只有一个活跃的 embedding model（`model_configs` 中 `model_type = 'embedding'` 且 `enabled = true` 的记录）。
2. `embeddings.model_config_id` 引用 `model_configs.id`，`embedding_dim` 从 model_configs 读取。
3. schema 中 `VECTOR` 列不固定维度——由应用层在创建时指定。pgvector 的 HNSW/IVFFlat 索引需要在部署时针对实际维度创建。
4. **禁止运行时随意切换 embedding 模型**。切换模型 = 触发全量重建索引快照，旧快照保留至新快照 activated 后清理。

### Phase 2+ 方向

1. 按 `model_config_id` 分 collection（独立表或 Qdrant collection），支持多模型并存过渡期。
2. 迁移到 Qdrant 后由 Qdrant 管理向量维度。
3. `index_snapshots.embedding_model_id` 记录快照绑定的模型，一致性检查时校验模型是否与当前配置一致。

## 6. 版本一致性

### 6.1 index_snapshots 统一快照

Chat 检索不再分散判断 page_version、chunk status、graphify_run，而是通过 `index_snapshots` 表统一管理：

```text
spaces.active_index_snapshot_id → index_snapshots.id
  绑定: graphify_run_id, wiki_repo_commit_hash, embedding_model_id
  关联: wiki_chunks.index_snapshot_id = spaces.active_index_snapshot_id
        graph_nodes.graphify_run_id = index_snapshots.graphify_run_id
```

Chat 检索约束简化为：

```sql
SELECT c.* FROM wiki_chunks c
JOIN spaces s ON c.space_id = s.id
WHERE c.index_snapshot_id = s.active_index_snapshot_id
  AND c.index_status = 'indexed'
```

### 6.2 快照生命周期

```text
building → ready → activated → superseded
```

- `building`：indexer-worker 正在写入 chunk 和 embedding
- `ready`：所有 chunk 已索引，待激活
- `activated`：写入 `spaces.active_index_snapshot_id`，Chat 开始使用
- `superseded`：被新快照替代，旧 chunk 可按保留策略清理

### 6.3 Fallback

如果当前快照构建失败或不存在，Chat 使用 `spaces.active_index_snapshot_id` 指向的上一个成功快照。不会因索引正在构建而阻断问答。

## 7. 数据保留策略

| 数据 | 保留策略 |
|---|---|
| 页面版本 | 默认永久，支持管理员清理旧版本。 |
| Source Archive | 默认永久，受合规策略控制。 |
| Graphify 输出 | 保留最近 N 次完整输出，旧输出归档。 |
| Embedding | 页面版本废弃后可清理。 |
| Chat 消息 | 按组织策略。 |
| 审计日志 | 至少 180 天，建议 1 年以上。 |

## 8. SQL 草案

完整草案见 [`../schemas/schema.sql`](../schemas/schema.sql)。

## 9. Phase 1 验收

1. 表结构支持 Space 隔离。
2. Wiki 页面支持版本化。
3. Source Document 只作为归档。
4. Graphify Run 可追踪输出。
5. 节点、边、chunk 都可绑定页面版本和 ACL。
6. Chat 检索不会返回未发布或无权限对象。

## 8. 版本一致性补充

> 本节合并 TODO T-5.1.2 / T-7.1。

### 8.1 `spaces` 新增字段

| 字段 | 类型 | 说明 |
|---|---|---|
| `index_consistency_status` | text | `healthy / checking / inconsistent`，由一致性检查任务写入。 |
| `active_graphify_run_id` | text | 当前 Space 最近一次成功或活跃 Graphify run。 |
| `active_index_snapshot_id` | text | 当前 Chat 使用的索引快照。指向 `index_snapshots.id`。 |

### 8.2 `wiki_pages` 新增字段

| 字段 | 类型 | 说明 |
|---|---|---|
| `indexed_version_id` | text | 最近一次成功索引的页面版本。Chat 默认读取该版本。 |
| `sync_status` | text | `synced / sync_pending / conflict_required / failed`。 |

### 8.3 版本锁索引

```sql
CREATE INDEX idx_wiki_pages_indexed_version ON wiki_pages(indexed_version_id);
CREATE INDEX idx_wiki_pages_current_indexed ON wiki_pages(current_version_id, indexed_version_id);
CREATE INDEX idx_spaces_consistency ON spaces(index_consistency_status);
CREATE INDEX idx_wiki_versions_status ON wiki_page_versions(tenant_id, space_id, status, created_at DESC);
```

### 8.4 tenant_id 约束

所有核心业务表必须具备 `tenant_id`：

- 用户、组、Space、权限。
- Wiki 页面、版本、chunk、embedding 关联对象。
- Source Archive、jobs、graphify_runs。
- graph_nodes、graph_edges、graph_communities、graph_reports。
- chat、audit、feedback、proposals。

即使当前是内部小团队单 tenant，也保留该字段以便权限隔离、审计和未来扩展。

## 9. GraphRAG 扩展表建议

TODO 之外建议增加以下表，便于 Phase 3-4：

| 表 | 阶段 | 用途 |
|---|---|---|
| `model_configs` | Phase 1 | 模型配置管理（chat/embedding/rerank/vision），含 embedding_dim。 |
| `page_block_metadata` | Phase 2 | 区块所有权 sidecar，防 Docmost 富文本清洗 HTML 注释。 |
| `wiki_sections` | Phase 1 | 页面段落索引，支持段落级引用和 anchor 跳转。 |
| `source_links` | Phase 1 | Wiki 页面/段落到源文档的证据链映射。 |
| `answer_citations` | Phase 1 | Chat 回答的结构化引用记录（替代 citations_json 做主表）。 |
| `index_snapshots` | Phase 1 | 统一索引快照，绑定 graphify_run、commit_hash、embedding_model。 |
| `graph_node_aliases` | Phase 1 | 节点别名追踪（见 Doc 21 §8A.5）。 |
| `graph_node_merges` | Phase 4 | 管理员节点合并（见 Doc 21 §8A.6）。 |
| `graph_communities` | Phase 3 | 存储 Graphify 社区、摘要、god node 关联。 |
| `wiki_update_proposals` | Phase 2 | 存储 Graphify 候选更新和人工审核状态。 |
| `consistency_checks` | Phase 2 | 记录每次一致性检查结果。 |
| `retrieval_traces` | Phase 3 | 记录 GraphRAG 检索候选、rerank、引用链。 |
| `feedback_items` | Phase 4 | 用户反馈与知识治理闭环。 |
| `bridge_events` | Phase 2 | Docmost webhook 幂等接收记录（event_id 去重、签名校验、处理状态）。 |
| `webhook_deliveries` | Phase 2 | Cherry → 外部 webhook 投递跟踪（重试、状态码、投递确认）。 |
| `graph_evidence_refs` | Phase 3 | graph_edges 证据引用规范化（支持反向查询"哪些边引用了此页面"）。 |
| `permission_versions` | Phase 2 | ACL 变更历史（审计、回滚、权限过期追溯）。 |
| `api_tokens` | Phase 4 | 用户 API token（用于外部集成和 MCP Gateway 认证）。 |
| `sessions` | Phase 1 | refresh_token 会话管理（支持多设备、强制登出）。 |
| `model_usage_logs` | Phase 3 | 模型调用用量记录（token 计量、延迟追踪、成本分析）。 |
| `system_settings` | Phase 1 | 系统级配置 KV 存储（替代硬编码配置，支持运行时变更）。 |
