...
- Client-facing local string codes promoted to `ErrorCode`: `NO_EMBEDDING_MODEL_CONFIGURED`, `RETRIEVAL_TRACE_NOT_FOUND`, `REBUILD_ALREADY_RUNNING`, `INVALID_SCOPE`, `PROPOSAL_ALREADY_RESOLVED`, `INVALID_PROPOSAL_STATUS`, `INVALID_PROPOSAL_ACTION`, `REINDEX_ALREADY_RUNNING`.
- Local string codes intentionally not promoted: `WORKER_TIMEOUT`, because it is stored in internal job `error_json` for dead-worker cleanup and is not thrown through the API error envelope.
...
| jobs | `apps/api/src/jobs/jobs.service.ts:542` | N/A | 401, 404, 409, 422 | `apps/api/src/jobs/__tests__/jobs.service.test.ts` | Public job API uses shared generic codes. Internal job helper is listed separately below. |
...
| internal/jobs | `apps/api/src/internal/internal-jobs.service.ts:994` | N/A for thrown API errors | 404, 409, 500 | `apps/api/src/internal/__tests__/internal-jobs.service.test.ts` | `WORKER_TIMEOUT` is a non-client job payload string in `error_json`, not an API envelope code, so it is out of scope for promotion. |
