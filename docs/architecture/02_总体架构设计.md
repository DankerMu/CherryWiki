...
sequenceDiagram
  ...
  participant CC as Claude Code（persistent stream-json）
  ...
  API->>CC: spawn claude -p --input-format stream-json --session-id <uuid>
  API->>CC: stdin 写入 user JSON line
  loop 多轮 tool-use 循环
  ...
  API-->>CW: SSE 转发
  CC-->>API: result；进程保持存活并进入 idle（捕获 provider_session_id）
  CW-->>U: 展示回答、引用、图表

  Note over U, CC: 后续消息优先复用 idle process；API 重启/idle kill 后 --resume provider_session_id

  U->>CW: 消息 N+1（追问）
  CW->>API: POST /chat/completions
  API->>SM: 查找已绑定的 agent_sessions 元数据
  SM-->>API: 返回 live process 或 provider_session_id
  API->>API: 每 turn 刷新 CLAUDE.md/settings.json；fingerprint 变化则 restart + resume
  API->>CC: stdin 写入追问；若无 live process 则 spawn --resume provider_session_id
  CC-->>API: stdout 流式回答（有上文记忆）
  CC-->>API: result 后回到 idle
  API-->>CW: SSE 转发
  CW-->>U: 展示回答
```
...
| 当前会话已绑定 Agent session | — | 是（live process 复用；无 live process 时 `--resume <provider_session_id>`） |
