...
URL_FETCH_WORKER_PROTOCOL = WorkerProtocolConfig(
    job_type="url_fetch",
    worker_type="url_fetch",
    worker_id_prefix="url-fetcher-worker",
    failure_log_message="url_fetch job failed",
    worker_error_type=UrlFetchJobError,
...
class InternalApiClient(_SharedInternalApiClient):
    def fetch_pending_job(
        self, *, job_type: str = URL_FETCH_WORKER_PROTOCOL.job_type
    ) -> dict[str, Any] | None:
    ...
    def heartbeat(
        ...
        *,
        worker_type: str = URL_FETCH_WORKER_PROTOCOL.worker_type,
    ) -> dict[str, Any]:
...
def generate_worker_id() -> str:
    return _generate_worker_id(URL_FETCH_WORKER_PROTOCOL)
...
def run_once(
    *,
    ...
    return _run_once(
        config=URL_FETCH_WORKER_PROTOCOL,
